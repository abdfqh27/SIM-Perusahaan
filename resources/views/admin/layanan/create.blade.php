...
<style>
    ...
    .auto-urutan-info i { font-size: 2rem; color: #2196F3; flex-shrink: 0; }
    .auto-urutan-info div { flex: 1; }
    .auto-urutan-info h4 { margin: 0 0 0.25rem 0; color: var(--blue-dark); font-size: 1rem; }
    .auto-urutan-info p  { margin: 0; color: #6c757d; font-size: 0.875rem; }
    .auto-urutan-info .urutan-number { font-size: 2rem; font-weight: 700; color: var(--orange-primary); flex-shrink: 0; }

    /* Char counter */
    .char-counter { display: flex; justify-content: flex-end; margin-top: 0.25rem; font-size: 0.8rem; color: #6c757d; }
    ...
    <form action="{{ route('admin.layanan.store') }}" method="POST" enctype="multipart/form-data" id="layananForm" novalidate>
        ...
        <div class="form-group">
            ...
            <textarea name="deskripsi_singkat" id="deskripsi_singkat" rows="3"
                      ...
                      maxlength="500" required>{{ old('deskripsi_singkat') }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea name="deskripsi_lengkap" id="deskripsi_lengkap" rows="6"
        ...
        <div class="form-group">
            ...
            <div class="fasilitas-container" id="fasilitasContainer">
                @if(old('fasilitas'))
                    @foreach(old('fasilitas') as $fas)
                    <div class="fasilitas-item">
                        <input type="text" name="fasilitas[]" class="form-control"
                               placeholder="Nama fasilitas" value="{{ $fas }}" maxlength="255">
                ...
                @else
                    <div class="fasilitas-item">
                        <input type="text" name="fasilitas[]" class="form-control"
                               placeholder="Nama fasilitas" maxlength="255">
                        <button type="button" class="btn-remove-fasilitas" onclick="removeFasilitas(this)">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('deskripsi_singkat').addEventListener('input', function () {
        ...
        document.getElementById('singkatCounter').className =
            'char-counter' + (len >= 500 ? ' danger' : len >= 400 ? ' warning' : '');
    });
    ...
    document.getElementById('addFasilitas').addEventListener('click', function () {
        ...
        item.innerHTML = `
            <input type="text" name="fasilitas[]" class="form-control" placeholder="Nama fasilitas" maxlength="255">
    ...
    layananForm.addEventListener('submit', function (e) {
        ...
        // Cek field wajib satu per satu — stop di yang pertama kosong
        const checks = [
        ...
        if (!imageInput.files.length) {
            Swal.fire({ icon: 'error', title: 'Gambar Wajib Diupload', text: 'Silakan upload gambar terlebih dahulu!' });
            imageUploadArea.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return;
        }

        // Cek fasilitas kosong
        const emptyFas = [...document.querySelectorAll('#fasilitasContainer input[name="fasilitas[]"]')]
            .find(i => !i.value.trim());
        if (emptyFas) {
            emptyFas.focus();
            emptyFas.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return;
        }
        ...
        Swal.fire({
            ...
            html: `Apakah Anda yakin ingin menyimpan layanan ini?<br><small class="text-muted">Layanan akan ditempatkan pada urutan ke-<strong>{{ $nextUrutan }}</strong>.</small>`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    title: 'Menyimpan...',
                    html: 'Sedang mengupload dan memproses data layanan',
